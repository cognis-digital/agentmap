...
**agentmap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `agentmap` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
agentmap ... --format json | cognis-connect emit --to sigma   # Sigma rules
agentmap ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
agentmap ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `agentmap` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="agentmap") for rec in records]   # records = your JSON output
